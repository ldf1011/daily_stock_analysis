...
class BacktestRepository:
    ...
    def get_candidates(
        ...
        *,
        ...
        with self.db.get_session() as session:
            ...
            if code:
                conditions.append(self._build_code_condition(AnalysisHistory.code, code))
            conditions.append(
                or_(
    ...
    def get_summary(
        ...
        *,
        ...
        with self.db.get_session() as session:
            ...
            if code is not None:
                conditions.append(self._build_code_condition(BacktestSummary.code, code))
    ...
    def _build_result_conditions(
        *,
        ...
        conditions = []
        if code:
            conditions.append(BacktestRepository._build_code_condition(BacktestResult.code, code))
        if eval_window_days is not None:
    ...
    @staticmethod
    def _build_code_condition(column, code: str):
        """Match either bare codes or exchange-suffixed variants.

        Historical rows in this app are not fully normalized yet: some are stored as
        ``601231.SH`` while others are stored as ``601231``. Accepting both makes
        the backtest filters tolerant to whichever shape the user types.
        """
        raw = (code or "").strip()
        normalized = normalize_stock_code(raw)
        variants = {value for value in (raw, raw.upper(), normalized) if value}
        conditions = [column.in_(sorted(variants))]
        if normalized and normalized.isdigit():
            conditions.append(column.like(f"{normalized}.%"))
