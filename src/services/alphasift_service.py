...
_DSA_CUSTOM_ALPHASIFT_STRATEGIES: Tuple[Dict[str, Any], ...] = (
    {
        "id": "donchian_breakout",
        "name": "唐奇安通道突破",
        "title": "唐奇安通道突破",
        "description": "基于 20 日通道上破/下破的顺势突破策略，偏趋势追踪，弹性大但回撤也更明显。",
        "category": "trend",
        "tag": "trend",
        "tags": ["trend", "breakout"],
        "market_scope": ["cn"],
    ...
    },
    {
        ...
        "name": "RSI综合优选",
        "title": "RSI综合优选",
        "description": "结合 RSI 超卖修复、价格反弹、均线过滤与量能条件，偏向低位修复后的右侧确认。",
        "category": "reversal",
        "tag": "reversal",
        "tags": ["reversal", "rsi"],
        "market_scope": ["cn"],
    ...
    },
    {
        ...
        "name": "威科夫增强",
        "title": "威科夫增强",
        "description": "围绕吸筹区、Spring、SOS 与 LPS 的结构化识别，适合跟踪主力吸筹后的趋势启动。",
        "category": "framework",
        "tag": "framework",
        "tags": ["framework", "wyckoff"],
        "market_scope": ["cn"],
        "market": "cn",
    },
)

_DSA_CUSTOM_SCREENING_STRATEGY_ALIASES: Dict[str, str] = {
    "donchian_breakout": "volume_breakout",
    "rsi_composite": "oversold_reversal",
    "wyckoff_enhanced": "shrink_pullback",
}
...
class AlphaSiftService:
    ...
    def screen(self, *, strategy: str, market: str, max_results: int) -> Dict[str, Any]:
        ...
        effective_strategy = _resolve_screening_strategy_alias(strategy)

        adapter = _get_dsa_adapter()
        screen = _get_adapter_callable(adapter, "screen", "screen() 不可调用。")
        try:
            raw = _call_alphasift_screen(screen, effective_strategy, market, max_results, self.config)
        ...
        if effective_strategy != strategy:
            warnings.append(
                f"Strategy alias applied: requested={strategy}, upstream_candidate_pool={effective_strategy}"
            )
        return {
            "enabled": True,
            "candidates": selected,
            ...
            "strategy": strategy,
            "upstream_strategy": raw_data.get("strategy") or effective_strategy,
...
def _list_strategies() -> List[Dict[str, Any]]:
    ...
    return _merge_custom_strategy_catalog(normalized)


def _merge_custom_strategy_catalog(strategies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in _DSA_CUSTOM_ALPHASIFT_STRATEGIES:
        strategy = _normalize_strategy(item)
...
def _resolve_screening_strategy_alias(strategy: str) -> str:
    strategy_id = str(strategy or "").strip()
    if not strategy_id:
        return strategy_id
    return _DSA_CUSTOM_SCREENING_STRATEGY_ALIASES.get(strategy_id, strategy_id)
...
def _call_alphasift_screen(screen: Any, strategy: str, market: str, max_results: int, config: Config) -> Any:
    ...
    with (
        ...
        _alphasift_soft_daily_filter_fallback(fallback_state),
        _alphasift_litellm_headers(config),
    ):
        try:
            result = screen(strategy, **kwargs)
            ...
            if not (supports_var_kwargs or supports_var_positional or len(positional_params) >= 3):
                raise exc
            result = screen(strategy, market, max_results)

    if fallback_state["used"] and isinstance(result, dict):
        warnings = result.get("warnings")
        if not isinstance(warnings, list):
            warnings = []
        warnings.append("Daily hard filter produced no survivors; kept snapshot-filtered candidates and flagged the result for review.")
...
@contextmanager
def _alphasift_soft_daily_filter_fallback(fallback_state: Dict[str, bool]) -> Iterator[None]:
    try:
        pipeline_module = importlib.import_module("alphasift.pipeline")
        filter_module = importlib.import_module("alphasift.filter")
    except Exception:
        yield
        return

    original_apply_hard_filters = getattr(pipeline_module, "apply_hard_filters", None)
    requires_daily_features = getattr(filter_module, "requires_daily_features", None)
    without_daily_filters = getattr(filter_module, "without_daily_filters", None)
    if not callable(original_apply_hard_filters) or not callable(requires_daily_features) or not callable(without_daily_filters):
        yield
        return

    def apply_hard_filters_with_fallback(df: Any, filters: Any) -> Any:
        result = original_apply_hard_filters(df, filters)
        try:
            if getattr(result, "empty", True) and not getattr(df, "empty", True) and requires_daily_features(filters):
                fallback = original_apply_hard_filters(df, without_daily_filters(filters))
                if not getattr(fallback, "empty", True):
                    fallback_state["used"] = True
                    return fallback
        except Exception as exc:
            logger.debug("AlphaSift daily hard filter fallback skipped: %s", exc)
        return result

    with _ALPHASIFT_RUNTIME_ENV_LOCK:
        setattr(pipeline_module, "apply_hard_filters", apply_hard_filters_with_fallback)
        try:
            yield
        finally:
            setattr(pipeline_module, "apply_hard_filters", original_apply_hard_filters)
