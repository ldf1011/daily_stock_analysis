...
export const backtestApi = {
  ...
  simulateStrategy: async (params: StrategyBacktestRequest): Promise<StrategySimulationResponse> => {
    const requestData: Record<string, unknown> = {
      code: params.code,
      strategy: params.strategy || 'momentum_quality',
    };
    if (params.startDate) requestData.start_date = params.startDate;
    if (params.endDate) requestData.end_date = params.endDate;
    if (params.initialCash != null) requestData.initial_cash = params.initialCash;
    if (params.stopLossPct != null) requestData.stop_loss_pct = params.stopLossPct;
    if (params.takeProfitPct != null) requestData.take_profit_pct = params.takeProfitPct;

    const response = await apiClient.post<Record<string, unknown>>(
      '/api/v1/backtest/strategy/simulate',
      requestData,
    );
    const data = toCamelCase<StrategySimulationResponse>(response.data);
    return {
      ...data,
      trades: (data.trades || []).map(item => toCamelCase(item)),
    };
  },
