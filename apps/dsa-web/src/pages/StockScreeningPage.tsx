...
const SCREEN_TASK_MAX_AGE_MS = 8 * 60 * 1000;
...
const readPersistedScreenTask = (): PersistedScreenTask | null => {
  ...
  try {
    ...
    return {
      ...
      submittedAt: Number.isFinite(Number(parsed.submittedAt)) ? Number(parsed.submittedAt) : undefined,
    };
  } catch {
    return null;
  }
...
const formatScreenMessage = (value: string) => {
  ...
  const strategyAlias = value.match(/^Strategy alias applied:\s*requested=([a-zA-Z0-9_-]+),\s*upstream_candidate_pool=([a-zA-Z0-9_-]+)$/i);
  if (strategyAlias) {
    return `当前策略使用兼容候选池：${strategyAlias[1]} -> ${strategyAlias[2]}，用于避免底层选股引擎直接报错。`;
  }
...
const StockScreeningPage: React.FC = () => {
  ...
  const [activeTaskStartedAt, setActiveTaskStartedAt] = useState<number | null>(restoredTask?.submittedAt ?? null);
  ...
  useEffect(() => {
    ...
    const startedAt = activeTaskStartedAt ?? Date.now();

    function finishTask(nextError?: string) {
    ...
    function applyTaskStatus(task: AlphaSiftScreenTaskStatus) {
      ...
      if (isRunningScreenTask(task.status)) {
        if (Date.now() - startedAt > SCREEN_TASK_MAX_AGE_MS) {
          setCandidates([]);
          setScreenMeta(null);
          setExpandedCode(null);
          finishTask('选股任务运行时间过长，已停止自动轮询。后台任务可能仍在继续，请稍后刷新重试。');
          return;
        }
        setLoading(true);
  ...
  }, [activeTaskId, activeTaskStartedAt, applyScreenResult]);
