...
describe('BacktestPage', () => {
  ...
  it('shows a maturity warning when the run only has insufficient samples', async () => {
    mockGetOverallPerformance.mockResolvedValueOnce({
      ...basePerformance,
      totalEvaluations: 1,
      completedCount: 0,
      insufficientCount: 1,
      directionAccuracyPct: undefined,
      winRatePct: undefined,
      neutralRatePct: undefined,
      avgStockReturnPct: undefined,
      avgSimulatedReturnPct: undefined,
      ...
      items: [
        {
          ...
          evaluatedAt: '2026-06-13T17:24:55.934833',
          outcome: undefined,
          directionCorrect: undefined,
          actualMovement: undefined,
    ...
    expect(await screen.findByText('等待数据成熟')).toBeInTheDocument();
    expect(screen.getByText(/当前只有 1 条待成熟样本/)).toBeInTheDocument();
    expect(screen.getByText('当前没有正在运行的回测任务')).toBeInTheDocument();
    expect(screen.getByText(/样本的分析日是 2026-06-12/)).toBeInTheDocument();
    expect(screen.getByText(/想立刻看到可完成样本/)).toBeInTheDocument();
  ...
  it('explains when a backtest run finished with only insufficient samples', async () => {
    ...
    expect(await screen.findByText('回测已结束：等待未来行情成熟')).toBeInTheDocument();
    expect(screen.getByText(/不是卡住/)).toBeInTheDocument();
  });
