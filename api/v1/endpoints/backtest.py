...
@router.post(
    ...
    responses={
        200: {"description": "历史策略模拟完成"},
        400: {"description": "请求参数错误", "model": ErrorResponse},
        500: {"description": "服务器错误", "model": ErrorResponse},
    },
    summary="历史策略模拟回测",
    description="使用 stock_daily 日线数据直接模拟策略买卖，独立于历史 AI 分析验收回测。",
)
def simulate_strategy_backtest(
    ...
    try:
        ...
        logger.error(f"历史策略模拟失败: {exc}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={"error": "internal_error", "message": f"历史策略模拟失败: {str(exc)}"},
        )
